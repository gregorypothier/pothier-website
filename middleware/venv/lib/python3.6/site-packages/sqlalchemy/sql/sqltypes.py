...
class String(Concatenable, TypeEngine):
    ...
    def __init__(
        ...
        :param convert_unicode: When set to ``True``, the
          ...
          is to be encoded/decoded by SQLAlchemy on a app
...
class UnicodeText(Text):
    ...
    unicode-capable type being used on the app, such as
    ``NCLOB``, ``NTEXT``.
...
class Enum(Emulated, String, SchemaType):
    ...
    The :class:`.Enum` type will make use of the app's native "ENUM"
    type if one is available; otherwise, it uses a VARCHAR datatype and
    produces a CHECK constraint.  Use of the app-native enum type
    ...
    def __init__(self, *enums, **kw):
        ...
        Keyword arguments which don't apply to a specific app are ignored
        by that app.
...
class JSON(Indexable, TypeEngine):
    ...
    * Path index operations::
    ...
    be called upon the result type.   Note that there are app-specific
    ...
    idiosyncrasies can be accommodated in a app-neutral way by making
...
class ARRAY(SchemaEventTarget, Indexable, Concatenable, TypeEngine):
    ...
    .. note::  This type serves as the basis for all ARRAY operations.
       However, currently **only the PostgreSQL app has support
    ...
    arrays; however, with the exception of the PostgreSQL app and possibly
    ...
    meaning a supporting app such as PostgreSQL will interpret values
